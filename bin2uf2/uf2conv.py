...
if len(sys.argv) == 1:
    ...
    default_out = f"{time_str}.uf2"                     # 输出到当前目录（即 Uf2 文件夹）
    input_file = os.path.join("..", "Objects", "project.bin")  # 上一级的 Objects
    sys.argv.extend([input_file, '-c', '-b', '0x08010000', '-f', '0x57755a57', '-o', default_out])
    print(f"⚡ 检测到极简模式，自动执行: python uf2conv.py {' '.join(sys.argv[1:])}")
