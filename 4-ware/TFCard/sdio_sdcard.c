...
SD_Error SD_Init(void)
{
	...
	xSDcardSemaphore = xSemaphoreCreateCounting(2,0);
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA2, ENABLE);//使能GPIOB, DMA2时钟
	...
	GPIO_Init(GPIOC, &GPIO_InitStructure);//复用功能输出
	...
	if(!g_TFcard_status) errorstatus = SD_NO_CARD;
	if(errorstatus==SD_OK) errorstatus = SD_PowerON();	 //SD卡上电
...
SD_Error SD_WriteBlock(uint8_t *buf,long long addr,  uint16_t blksize)
{
	...
	cardstatus=SDIO->RESP1;													  
   	...
   	while((cardstatus&0x00000100)==0) 	//检查READY_FOR_DATA位是否置位
	{
		SDIO_CmdInitStructure.SDIO_Argument = (uint32_t)RCA<<16;//发送CMD13,查询卡的状态,短响应
		SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SEND_STATUS;
		SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
